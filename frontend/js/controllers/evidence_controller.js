      ...
      content: `
        <div style="display:flex; flex-direction:column; gap:12px;">
          <div style="background:var(--bg-subtle); padding:10px 14px; border-radius:var(--radius-sm); font-size:12px; display:grid; grid-template-columns:1fr 1fr; gap:8px;">
            <div><strong>发文机关:</strong> ${item.promulgation}</div>
            <div><strong>文件字号:</strong> ${item.docNo}</div>
            <div><strong>时效状态:</strong> <span class="badge badge-success">${item.validity}</span></div>
            <div><strong>命中位置:</strong> ${item.loc}</div>
          </div>
          <div style="border:1px solid var(--border-light); border-radius:var(--radius-sm); padding:14px; background:#fff; font-family:var(--font-code); white-space:pre-wrap; font-size:12px; max-height:360px; overflow-y:auto; line-height:1.7;">
            ${item.previewSnippet}
          </div>
        </div>
      `
