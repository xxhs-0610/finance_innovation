...
def _resolve_default_db_path(requested: str | Path) -> Path:
    ...
    for candidate in [
        ...
        "data/processed/kb_full_validation/metadata.db",
        "data/processed/metadata.db",
        "data/processed/kb/metadata.db",
    ]:
        resolved_cand = resolve_path(candidate)
...
def _infer_vector_index_dir(db_path: Path) -> Path:
    ...
    if parent.name and parent.name not in {"processed", "data"}:
        return resolve_path(Path("indexes") / parent.name)
    return resolve_path(Path("indexes"))
